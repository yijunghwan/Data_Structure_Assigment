...
//이진 트리 자료구조 함수
void tree_free(binary_tree*);//정렬된 이진트리 메모리 해제 함수 (후위순회 사용)
binary_tree* sort(FILE*);//파일에서의 다항식 받아 이를 이진트리로 정렬

//1번 자료구조 함수
void RVL_1(binary_tree*, float*, int*);//1번 자료구조를 만드는 함수(배열 자료구조)(중위순회 사용)
float big_exp(binary_tree*);//가장 큰 지수를 찾는 함수(이진트리에서)
float small_exp(binary_tree*);//가장 작은 지수를 찾는 함수(이진트리에서)
float* poly_add1(binary_tree*);//1번 배열 자료구조를 이용한 다항식 덧셈 출력 포함

//2번 자료구조 함수
void RVL_2(binary_tree*, polynomial_2*, int*);//2번 자료구조를 만드는 함수(배열 개선 자료구조)(중위순회 사용)
polynomial_2* poly_add2(binary_tree*);//2번 배열 개선 자료구조를 이용한 다항식 덧셈 출력 포함

//3번 자료구조 함수
void RVL_3(binary_tree*, polynomial_3*);//3번 자료구조를 만드는 함수(연결리스트 자료구조)(중위순회 사용)!!!!마지막에 빈노드가 추가되는 문제 있음
void last_node_pop(polynomial_3*);//연결리스트의 마지막 노드 삭제 함수 RVL_3의 문제 해결 방안
void linked_list_free(polynomial_3*);//3번 연결리스트 자료구조 메모리 해제 함수
polynomial_3* poly_add3(binary_tree*);//3번 연결리스트 자료구조를 이용한 다항식 덧셈 출력 포함

//파일 입출력 함수
void outpu_time_file(FILE*, clock_t[]);//출력 파일 쓰기 함수 (시간 측정 결과)
...
int main()
{
	make_poly();//다항식 생성 함수 호출
	FILE* input;//입력 파일 포인터
	input = fopen("Input_file_format.txt", "r");//입력 파일 열기
	if (input == NULL)//오류처리 
	{
		printf("입력 파일을 열 수 없습니다.\n");
		return -1;
	}
	output = fopen("output_file_format.txt", "w");//출력 파일 열기
	if (output == NULL)//오류처리
	{
	...
	//*/
	outpu_time_file(output, time);//출력 파일 쓰기 함수 호출
...
* 이 자료구조를 이용한 아래 함수의 시간복잡도는 O(log n) n='1번 다항식의 항의 개수'
*/
...
binary_tree* sort(FILE* input)//파일에서의 다항식 받아 이를 이진트리로 정렬하는 함수
{
	...
	if (num1 > 0)//num1이 0보다 크면 즉 다항식이 존재하면
	{
		...
		for (int i = 1; i < num1; i++)//모든 항을 저장하기위한 반복문(이미 헤드노드에 하나의 항이 저장되었으니 1부터 시작) 
		{
			...
			while (1)//새로운 노드 위치 찾을 고 생성할 때까지 반복
			{
				...
				{
					printf("error: 1번 다항식 정렬 중 오류 (exp: %d, coef: %d)\n", exp, coef);//에러메세지 출력
					exit(1);//종료
				}
			}
		}
	...
	if (num2 > 0)//num2이 0보다 크면 즉 다항식이 존재하면
	{
		...
		for (int i = 1; i < num2; i++)//모든 항을 저장하기위한 반복문(이미 헤드노드에 하나의 항이 저장되었으니 1부터 시작) 
		{
			...
			while (1)//새로운 노드 위치 찾을 고 생성할 때까지 반복
			{
				...
				{
					printf("error: 2번 다항식 정렬 중 오류 (exp: %d, coef: %d)\n", exp, coef);//에러메세지 출력
					exit(1);//종료
				}
			}
		}
...
* 즉 공간 복잡도 빅오 기준 O(n) n='가장 큰 지수'이다
*
* 이 자료구조를 이용한 아래 함수의 시간복잡도의 경우는
* RVL_1함수가 O(log n)의 시간 복잡도이지만 합산은 O(n)이다
* 즉 시간 복잡도는 빅오 기준 O(n) n='가장 큰 지수'이다
*
*
* 추가로
...
void RVL_1(binary_tree* node, float* coe, int* i)
{
	...
	{
		printf("error: RVL_1 최대차수 오버");//에러메세지 출력
		exit(1);//종료
	}
}
...
float* poly_add1(binary_tree* head)
{
	...
	float iii = big_exp(head->left);//1번 다항식의 가장큰 지수
...
* 자료구조
* 설명:
* 실수와 정수로 이루어진 2중배열(리스트)
* poly[i].coef='계수' poly[i].exp='지수' (i<'1번 항과 2번항의 개수의 합')
* 배열의 사이즈는('총 항의수')*2byte = ('1번 항과 2번항의 개수의 합')*(1byte 1+byrte '계수'+'지수') = 2n
* 즉 공간 복잡도는 빅오 기준 O(n) n='1번 항과 2번항의 개수의 합'
*
* 시공간 복잡도:
* 이 자료구조를 이용한 아래 함수의 시간복잡의 경우는
* RVL_2함수가 O(log n)의 시간 복잡도이지만 합산은  O(n)이다
* 즉 시간 복잡도는 빅오 기준 O(n) n='1번 항과 2번항의 개수의 합'이다
*
*
* 추가로
* 기존의 배열형식의 자료구조만 사용했음
...
* 즉 공간 복잡도는 빅오 기준 O(n) n='총항의 개수의 합'
*
* 시공간 복잡도:
* 이 자료구조를 이용한 아래 함수의 시간복잡의 경우는
* 빅오 기준 O(n) n ='1번다항식 노드 수 + 2번 다항식 노드의 수'
*
*
* 추가로
...
void outpu_time_file(FILE* ouput,clock_t time[4])
{
	//시간 측정 결과 출력
	fprintf(ouput, "%f\t", (float)(time[1]) / CLOCKS_PER_SEC);//1번 자료구조 시간 측정 결과 출력
	fprintf(ouput, "%f\t", (float)(time[2]) / CLOCKS_PER_SEC);//2번 자료구조 시간 측정 결과 출력
	fprintf(ouput, "%f\t", (float)(time[3]) / CLOCKS_PER_SEC);//3번 자료구조 시간 측정 결과 출력
}
...
void make_poly()
{
	FILE* file = fopen("Input_file_format.txt", "w");
	if (file == NULL) 
	{
		printf("파일을 열 수 없습니다.\n");
	}

	int i = 10;//1번 다항식의 항의 개수
	int ii = 5;//2번 다항식의 항의 개수
	int iii = 10;//계수의 범위
	int iV = 10;//지수의 범위
	fprintf(file, "%d %d\n", i, ii);
	iV += 1;//0포함
