...
int big_exp(binary_tree*);//가장 큰 지수를 찾는 함수(이진트리에서) //매개변수 이진트리의 루트주소
int small_exp(binary_tree*);//가장 작은 지수를 찾는 함수(이진트리에서) //매개변수 이진트리의 루트주소
...
* 이 자료구조를 이용한 아래 함수의 시간복잡도는 보통 O(nlog n) 불균할 경우 최악 0(n^2) n='다항식의 항의 개수'
*/
...
*
* 시공간 복잡도:
* 배열의 사이즈는('가장 큰 지수의 수+2')		->					( 최고 지수 의수+ 저장용 지수 + 0인 계수 저장 )
...
int small_exp(binary_tree* tree)
{
	if (tree->left)
	{
...
float* poly_add1(binary_tree* head)
{
	...
	poly_d = (float*)calloc((int)(max_poly[0] + 2), sizeof(float));//결과 다항식 동적할당
...
*
* 시공간 복잡도:
*  배열의 사이즈는('총 항의수')*2byte = ('1번 항과 2번항의 개수의 합- 지수가 겹치는 항의 수')*(1byte 1+byrte '계수'+'지수') = 2n
* 즉 공간 복잡도는 O(n) n='1번 항과 2번항의 개수의 합-지수가 겹치는 항의 수'
...
* 즉 공간 복잡도는 O(n) n='총항의 개수의 합 - 지수가 겹치는 항의 개수'
